...
def load_latest_model_and_scaler(bucket_name, gcs_folder_name):
    storage_client = storage.Client() if is_running_in_gcp() else storage.Client.from_service_account_json(
    ...
    model_blobs = sorted([blob for blob in blobs if 'xgboost_model' in blob.name], key=lambda x: x.time_created,
                         reverse=True)
    scaler_blobs = sorted([blob for blob in blobs if 'scaler' in blob.name], key=lambda x: x.time_created, reverse=True)
    feature_names_blobs = sorted([blob for blob in blobs if 'feature_names' in blob.name], key=lambda x: x.time_created,
                                 reverse=True)
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        model_file = os.path.join(tmpdirname, latest_model_blob.name.split('/')[-1])
        scaler_file = os.path.join(tmpdirname, latest_scaler_blob.name.split('/')[-1])
        feature_names_file = os.path.join(tmpdirname, latest_feature_names_blob.name.split('/')[-1])
...
def load_data_from_bigquery(project_id: str, dataset_id: str, table_name: str) -> pd.DataFrame:
    ...
    client = bigquery.Client() if is_running_in_gcp() else bigquery.Client.from_service_account_json(
...
def predict_next_hours(model, scaler, latest_data, train_data, feature_names, hours=50):
    future_predictions = []
    current_time = latest_data['time_period_start'] + pd.Timedelta(
        hours=1)  # Start from the next hour after the latest data point
