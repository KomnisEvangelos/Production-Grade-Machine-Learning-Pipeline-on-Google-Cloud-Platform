...
def upload_df_to_bigquery(dataframe: pd.DataFrame, project_id: str, dataset_id: str, table_name: str,
    ...
    for col in dataframe.select_dtypes(['datetime64[ns, UTC]']).columns:
        dataframe[col] = dataframe[col].dt.tz_localize(None).astype('datetime64[us]')
...
def feature_engineering(df):
    ...
    df['price_open_avg_24h'] = df['price_open'].rolling(window=24,
                                                        min_periods=1).mean()  # 24-hour rolling average of opening price
    df['price_high_avg_24h'] = df['price_high'].rolling(window=24,
                                                        min_periods=1).mean()  # 24-hour rolling average of highest price
    df['price_low_avg_24h'] = df['price_low'].rolling(window=24,
                                                      min_periods=1).mean()  # 24-hour rolling average of lowest price
    df['volume_traded_avg_24h'] = df['volume_traded'].rolling(window=24,
                                                              min_periods=1).mean()  # 24-hour rolling average of traded volume
    df['trades_count_avg_24h'] = df['trades_count'].rolling(window=24,
                                                            min_periods=1).mean()  # 24-hour rolling average of trades count
    df['price_range_avg_24h'] = df['price_range'].rolling(window=24,
                                                          min_periods=1).mean()  # 24-hour rolling average of price range (high - low)
    df['price_change_avg_24h'] = df['price_change'].rolling(window=24,
                                                            min_periods=1).mean()  # 24-hour rolling average of price change (close - open)
    df['average_price_avg_24h'] = df['average_price'].rolling(window=24,
                                                              min_periods=1).mean()  # 24-hour rolling average of average price
    df['volatility_avg_24h'] = df['volatility'].rolling(window=24,
                                                        min_periods=1).mean()  # 24-hour rolling average of price volatility (standard deviation)
    df['trading_intensity_avg_24h'] = df['trading_intensity'].rolling(window=24,
                                                                      min_periods=1).mean()  # 24-hour rolling average of trading intensity (volume / trades count)

    # Calculate 7-day rolling average features
    df['price_open_avg_7d'] = df['price_open'].rolling(window=24 * 7,
                                                       min_periods=1).mean()  # 7-day rolling average of opening price
    df['price_high_avg_7d'] = df['price_high'].rolling(window=24 * 7,
                                                       min_periods=1).mean()  # 7-day rolling average of highest price
    df['price_low_avg_7d'] = df['price_low'].rolling(window=24 * 7,
                                                     min_periods=1).mean()  # 7-day rolling average of lowest price
    df['volume_traded_avg_7d'] = df['volume_traded'].rolling(window=24 * 7,
                                                             min_periods=1).mean()  # 7-day rolling average of traded volume
    df['trades_count_avg_7d'] = df['trades_count'].rolling(window=24 * 7,
                                                           min_periods=1).mean()  # 7-day rolling average of trades count
    df['price_range_avg_7d'] = df['price_range'].rolling(window=24 * 7,
                                                         min_periods=1).mean()  # 7-day rolling average of price range (high - low)
    df['price_change_avg_7d'] = df['price_change'].rolling(window=24 * 7,
                                                           min_periods=1).mean()  # 7-day rolling average of price change (close - open)
    df['average_price_avg_7d'] = df['average_price'].rolling(window=24 * 7,
                                                             min_periods=1).mean()  # 7-day rolling average of average price
    df['volatility_avg_7d'] = df['volatility'].rolling(window=24 * 7,
                                                       min_periods=1).mean()  # 7-day rolling average of price volatility (standard deviation)
    df['trading_intensity_avg_7d'] = df['trading_intensity'].rolling(window=24 * 7,
                                                                     min_periods=1).mean()  # 7-day rolling average of trading intensity (volume / trades count)
...
def main(request):
    ...
    latest_time_period_start = get_latest_time_period_start(settings.PROJECT_ID, settings.BQ_FEATURE_STORE_DATASET,
                                                            bq_table)
    ...
    upload_df_to_bigquery(dataframe=features, project_id=settings.PROJECT_ID,
                          dataset_id=settings.BQ_FEATURE_STORE_DATASET, table_name=bq_table, write_mode='WRITE_APPEND')
