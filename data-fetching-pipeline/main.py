...
def upload_json_to_gcs(json_data: dict, bucket_name: str, folder_path: str, symbol: str, start_date: str,
                       end_date: str) -> None:
...
def main(request):
    ...
    if latest_timestamp:
        print(
            f'There are already historical data saved in BigQuery until {latest_timestamp} for the selected Cryptocurrency')
    ...
    upload_json_to_gcs(all_data, bucket_name, folder_path, symbol, (end_date - timedelta(days=365 * 10)).isoformat(),
                       end_date.isoformat())
